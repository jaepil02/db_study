...
> **성격**: to-be 설계 정본이다. 학습 단계 S2(수직 슬라이스 — 수집 · 적재 · 최신값 · 시계열 raw · WebSocket · 웹 1페이지)까지 구현됐고(슬라이스 설비 1 · 태그 8과 티어 S 전 구성 설비 5 × 태그 50을 측정) S3 이후(파이프라인 심화 · 롤업 · 알람 · 인증)는 착수 전이며, 구현과 측정이 진행되면 각 문서를 as-built로 승격하고 미확인 수치를 EXP-NN 실측 결과로 갱신한다
