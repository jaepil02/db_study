...
- 문서군 구축은 웨이브 W0~W7로 진행한다. 진행 상태 · 웨이브 분담 · 착수 전 보정 결정의 정본은 저장소 루트 docs_plan.md다.
- **W0(골격)** — 본 문서 · [CLAUDE.md](./CLAUDE.md) · 폴더 README 12본 골격판. 고정 기준 · 전역 불변식 · ID 규약 · 문서 지도 · 읽는 순서를 고정한다. 이후 전 문서가 여기를 인용한다.
- 폴더 README는 W0에서 **골격판**으로 쓰고 소속 웨이브가 **완성판**으로 갱신한다. 골격판의 파일 목차는 확정 파일명이며, 아직 쓰지 않은 파일로 가는 링크는 W7 전수 검사에서 0건으로 닫는다.
- 원본 4본은 **W7까지 삭제하지 않는다** — 이관 누락을 검증할 원본이 필요하다.
...
| PostgreSQL 테이블 | 업무 **14** + 대조군 **1** = **15**. 업무 14 = site · production_line · device · modbus_config · tag_master · tag_master_history · alarm_rule · alarm_event · user_account · role · user_role · work_order · production_log · audit_log. 대조군 = plc_tag_raw_control(SW-09). 정본 [05_data_stores/01_postgresql_schema.md](./05_data_stores/01_postgresql_schema.md) · [05_data_stores/10_olap_vs_rdb_control.md](./05_data_stores/10_olap_vs_rdb_control.md) |
...
| Redis 영역 접두 | **9** — 봉인 계열(TTL 금지) 3 stream · rt · alarm + 캐시 계열(TTL 필수) 5 cache · lock · rl · sess · auth + Pub/Sub 채널 1 ch. 검산: 3 + 5 + 1 = **9**. 단일 인스턴스 · volatile-lru. 키 패턴 전수와 봉인 표의 정본은 [05_data_stores/05_redis_keyspace.md](./05_data_stores/05_redis_keyspace.md) |
