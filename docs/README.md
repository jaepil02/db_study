...
| 역할 스위치 | **11종** — SW-01~SW-11. Redis 역할 10(백프레셔 1 · 캐시 4 · 팬아웃 2 · 멱등 1 · 대조군 1 · 최신값 결합 1) + 수집 1(데드밴드). 검산: 1 + 4 + 2 + 1 + 1 + 1 + 1 = **11**. 기본값 on 8 · off 2(SW-09 · SW-10) · 구현 선택 1(SW-11 = ingest). 정본 [02_features/13_switch_matrix.md](./02_features/13_switch_matrix.md) |
...
| 채번 진행 중인 축 | API 표면 · 화면 코드(W5) · EXP-NN(W6). **수치는 정본이 채번한 같은 변경 단위에서 본 표에 행으로 올린다** — 정본보다 먼저 쓰지 않는다 |
