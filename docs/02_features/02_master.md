...
| **MST-03** | Modbus 접속 설정 관리 | 설비 1:1의 modbus_config(host · port · unit_id · timeout_ms · retry_count · max_regs_per_request)를 관리한다. Collector가 기동 시 PostgreSQL에서 읽고, 실행 중 변경은 커밋 뒤 ch:cacheinv 신호(cache:devlist:{site_id})로 다음 사이클 경계에 다시 읽는다(COL-01 · W4 판정). **host가 컨테이너 루프백이면 그 설비는 시뮬레이션 설비**이며 모드 A의 SIMULATED 표지가 이 값에서 나온다([03_collector.md](./03_collector.md) §모드 A의 SIMULATED 표지 판정) | S2(시드) · S4 | F-01 · F-05 | 해당 없음 | 07_api/04_master | PostgreSQL modbus_config |
...
| **MST-08** | 캐시 무효화 체인 | 무효화 체인 6단 — ① 트랜잭션 커밋 ② Redis 캐시 삭제 ③ ch:cacheinv 발행 ④ SYSTEM RELOAD DICTIONARY plc.dict_tag(tag_master 쓰기만 · 응답 뒤) ⑤ BFF 서버 fetch 캐시 태그 무효화 ⑥ 브라우저 쿼리 캐시 무효화 신호. ②~⑥은 커밋된 **뒤에만** 건다. modbus_config만 바꾸는 쓰기도 ③을 낸다(Collector 반영 — W4). 단 번호 정본 [../06_pipeline/07_business_crud.md](../06_pipeline/07_business_crud.md). 갱신이 아니라 **삭제**다 — 동시 갱신에서 쓰기 순서가 뒤집히면 낡은 값이 최종값으로 남는다. ⑤ · ⑥은 원본 보정 7.4가 더한 단이다 | S4 | F-05 | 해당 없음 | 표면 없음 — 쓰기 표면의 후처리 | Redis cache 계열 · ch:cacheinv · ClickHouse dict_tag |
