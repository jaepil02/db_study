...
| **OBS-06** | 스위치 상태 노출 | 현재 스위치 10종의 상태를 /api/v1/health 응답과 /metrics 레이블로 노출한다. 모든 측정 기록의 4요소 중 하나가 스위치 상태다(D-10) — 노출이 없으면 기록자가 환경변수를 손으로 옮겨 적다 틀린다. SW-01 off의 부팅 경고도 여기에 상태로 남긴다 | S2 | 해당 없음 — 관측 | SW-01~SW-10 전부(노출) | 07_api/10_metrics | 없음 |
