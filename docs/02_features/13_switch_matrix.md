...
> **대상**: 역할 스위치 10종의 채번 · 환경변수 · 기본값 · off · on 동작 · 측정 대상 · 교체되는 포트 · 관련 기능 · 흐름 · 원본 예상치 · 실험 자리 · 조합 제약 — SW-NN 채번 정본
> **작성일**: 2026-09-24
...
이 문서는 **SW-NN의 채번 정본**이다. 스위치는 기능이 아니라 기능의 구현을 바꿔 끼우는 **실험 손잡이**다 — 같은 코드에서 Redis의 각 역할(과 대조군 · 데드밴드)을 끄고 켜서 그 기여분을 분리한다(D-06). 스위치가 없으면 나중에 끼운 캐시의 전후 비교에 "코드가 달라져서"가 붙어 Redis 기여분을 분리할 수 없고, 처음부터 켠 캐시는 캐시가 없을 때의 고통을 영영 보여 주지 않는다(원본 implementation_plan.md §3.1).
...
SW-01~SW-10의 순서는 [README.md](./README.md) 고정 기준의 목록 순서 그대로다. 번호는 식별자이지 순서가 아니다 — 새 스위치는 SW-11부터 말미 채번한다.
...
| SW-02 | 점조회 비용 | LatestValuePort — RedisLatestValue · ClickHouseLatestValue | RLT-01 · RLT-02 | F-03 | 30~150 ms → 0.3~1 ms | 상동 — W6 채번 |
...
- **포트 이름은 SW-03(원본 implementation_plan.md §4.3 예시)을 뺀 아홉이 이 문서의 잠정 명칭이다.** 이름의 정본은 [../04_architecture/02_module_boundaries.md](../04_architecture/02_module_boundaries.md)(W3)이며, 이 표는 "포트 하나에 구현 둘"이라는 모양과 교체 대상 기능을 고정한다.
...
- 분류: 백프레셔 1(SW-01) + 캐시 4(SW-02 · 03 · 04 · 05) + 팬아웃 2(SW-06 · 07) + 멱등 1(SW-08) + 대조군 1(SW-09) + 수집 1(SW-10) = **10** — Redis 역할 1 + 4 + 2 + 1 + 1 = **9** · 수집 **1**
- 기본값: on 8(SW-01~08) + off 2(SW-09 · 10) = **10**
- 값 형식: 켜고 끄는 불리언 9 + 밀리초 1(SW-07 — 0이 off) = **10**
- 관련 흐름 참여(중복 허용): F-01 2(SW-01 · 10) + F-02 3(SW-01 · 08 · 09) + F-03 1 + F-04 3 + F-06 1 + F-07 2 + F-09 1 + F-10 1 = **14**. 스위치가 걸린 흐름 8 · 걸리지 않는 흐름 F-05 · F-08 — 10 − 8 = **2**(업무 CRUD의 무효화 체인과 롤업은 정합성 계약이라 스위치를 두지 않는다)
...
| 상태 노출 | 10종 전부를 /api/v1/health 응답과 /metrics 레이블로 노출한다(OBS-06) | 기록자가 환경변수를 손으로 옮겨 적다 틀린다 — 스위치 상태는 측정 기록 4요소의 하나다(D-10) |
| 기록 병기 | 모든 측정 수치에 10종의 상태를 적는다. 바꾼 것만 명시하고 나머지는 "기본값"으로 적을 수 있다 | 같은 조건이라 믿은 두 측정의 조건이 다르다 — SW-10이 빠지면 행 수가 다른 두 측정이 같은 실험으로 묶인다(D-08) |
...
| 항목 | 상태 | 확정 자리 |
|------|------|------|
| 포트 · 구현 이름(SW-03 제외 9) | 이 문서의 잠정 명칭 | [../04_architecture/02_module_boundaries.md](../04_architecture/02_module_boundaries.md)(W3) |
...
| SW-10 off와 경고 단계 데드밴드 강화(COL-08) | **신규 미확인** — 데드밴드가 꺼진 상태에서 "강화"의 뜻이 없다 | [../04_architecture/06_backpressure_failure.md](../04_architecture/06_backpressure_failure.md)(W3) |
...
- [README.md](./README.md) — 스위치 10종 목록 · DI 제약(축약)
