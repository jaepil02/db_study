...
| va | float64[] | 공학 단위 값 | 유한 값 — NaN · 무한대는 발행 전에 BAD_RANGE로 판정되고 값 자리는 NaN이면 0 · ±무한대면 ±Float64 최댓값이다 · 예외 응답(BAD_COMM)의 값 자리는 0(S3 as-built) | 상동 |
...
- **Stream 필드 이름은 p 하나다(S2 판정).** 엔트리는 XADD 필드 하나(p)에 msgpack 바이트 전체를 싣는다 — 계약 필드(v · d · s · t0 · tg · dt · va · q)를 Stream 필드로 펼치면 필드 이름이 엔트리마다 반복되어 컬럼 배열의 크기 이득이 사라지고, 발행자가 필드 순서를 바꾸는 것만으로 소비자 해석이 흔들린다. 소비자는 p가 없거나 해제되지 않는 엔트리를 해독 불가로 센다(S2는 DLQ가 없어 XACK · 로그 — ING-05는 S3).
