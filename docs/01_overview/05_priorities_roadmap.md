...
- **S0 합격(2026-09-24)** — AC-14 · AC-15가 3회 모두 성립했다(docs/measurements 기록 002 · EXP-29). 착수 체크리스트 1(Docker 메모리 상향) · 7(릴리스 노트 대조)이 미완인 채 S0에 들어갔다 — **진입 조건의 예외**다. S0 판정은 구조 판정뿐이라 개발 프로파일로 충분했다. 분포 판정 기록을 정본에 올리는 첫 단계(S2 첫 기록) 전에 둘을 이행한다. 저장소 판별 EXP-32(기록 001)는 ADR-14를 보강했다([../04_architecture/09_decision_records.md](../04_architecture/09_decision_records.md)).
