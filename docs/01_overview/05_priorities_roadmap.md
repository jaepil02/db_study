...
> **작성일**: 2026-09-24
> **개정일**: 2026-09-25 — S3 완료 반영(fdc7849 · EXP-29 기록 015 · EXP-13 기록 016 · EXP-14 기록 017 · EXP-19 기록 018 · EXP-31 기록 019 · EXP-34 기록 020) — S3 합격 불릿 신설 · S2 이미지 예외 닫힘 불릿 · 미확인 배치 세 안 파트 생성률 **닫힘** · Taskfile S3분 · 래퍼 용도별 메서드 완료
...
- **S3 합격(2026-09-25)** — 합격 판정 여섯이 전부 성립했다(fdc7849 · 부하 실험 · 스위치 기본값 — 팔이 바꾸는 스위치만 다르다). ① 무손실 — 티어 S 모드 A 9회(기본 · SW-09 on · SIM 계획 팔 각 3회) 모두 Stream 디코딩 포인트 = tag_raw 행 수(차 0 · 기록 015 · AC-01). 같은 9회에서 AC-02 중복 0 · AC-04 시각 차 0 ms · AC-05 세 해상도 불일치 0 · AC-07 전 태그 일치 · AC-08 품질 전파(예외 2 · 범위 4 · 타임아웃 행 없음 · 3 없음) · AC-13(보존 밖 일자 파티션 26~36초 안에 TTLDropMerge로 소멸 · mutation 0)도 성립했다. ② 랙 — 모드 A 9회의 1초 표본 1,725개 전부 그룹 lag 0 · pending 최대 8 · 정지 뒤 0(기록 015 · AC-19). 모드 B 티어 S 3회 중 1회는 발행 묶음과 표본이 겹친 순간 lag 1~2가 표본 3개(그중 2개 연속)에 잡히고 곧 0으로 돌아왔다(기록 020) — 발행 묶음(엔트리 5) 이하의 순간값이라 누적 없음으로 읽었고(리드 판정), 순간값과 누적을 가르는 문턱은 미확인으로 등재했다([../10_observability/01_metrics_catalog.md](../10_observability/01_metrics_catalog.md) §컨슈머 랙 판정). ③ SW-08 — 삽입 성공 뒤 XACK 전 강제 종료에서 off 3회 모두 중복 조합 250(재전달 배치 1개) · on 3회 모두 0(기록 016 · AC-20). **26.8은 deduplicate_insert가 insert_deduplicate를 대체해 off를 insert_deduplicate 0으로만 두면 중복이 재현되지 않았다** — off 구현을 deduplicate_insert 'disable'로 고쳐 닫았다. ④ 롤업 — 모드 B 3회 모두 1분 · 1시간 · 1일 버킷의 count · min · max · last 정확 일치 · avg 차가 상계식 안(차 ÷ 상계 최대 0.18 · 기록 019 · AC-05). ⑤ SW-09 — on 3회 모두 count(tag_raw) = count(plc_tag_raw_control)(차 0 · 기록 015 · AC-21). ⑥ 배치 세 안(티어 M 10,000 pps · 모드 B · 기록 020 · AC-22) — 파트 생성률 A · C 60/분(컨슈머 수와 무관 · 원본 산술 "독립 플러시 3배"가 창 정렬로 사라졌다) · B 12~12.5/분 · E2E p50 A · C 약 1,026 ms · B 약 3,051 ms. 곁 기록 — SW-10 프로파일 8종 off/on(기록 017 · AC-42) · DLQ(해독 불가 · 재시도 소진 → DLQ · PEL 0 · 정상분 차 0 · 기록 018 · AC-12).
- **S2 이미지 예외 닫힘(S3 착수 2026-09-25)** — 레지스트리 복구 뒤 d32b09a를 정식 빌드해 기록 012~014의 덮어쓰기 이미지와 /app/dist · node_modules 해시를 대조했고, 아무도 불러오지 않는 잔여 파일 하나(dist/modules/datagen/worker.js)만 달라 실행 코드가 같다 — 재측정하지 않는다(근거 docs/measurements/raw/s2-image-equivalence.txt · 기록은 고치지 않는다).
...
| 항목 | 내용 | 착수 단계 | 근거 · 정본 |
|------|------|------|------|
| Taskfile 기본 작업 | migrate · seed · snapshot · restore · bench | S0(스냅샷 · 복원 — **완료** 2026-09-24) · S3(migrate · seed 확장 — **완료** fdc7849 · migrate 관리자 단계 pg_partman · seed --range · --deadband · 끝에 사전 재적재) | 원본 tech_stack.md §11 · [../09_tech_stack/05_tooling_devops.md](../09_tech_stack/05_tooling_devops.md) |
...
| 키 계열별 래퍼 | 캐시 계열은 TTL 필수 파라미터 · 봉인 계열은 TTL 명령 비노출 · 실패 전략 이원화 | S2 — **완료**(fe64472 · CacheKeyClient · DurableKeyClient · FanoutPublisher) · 용도별 메서드 **S3 완료**(fdc7849 — 캐시 원시 메서드 비공개 · 조회 결과 · 태그 메타 · 재빌드 잠금 용도 메서드 · 봉인 계열 XAUTOCLAIM · DLQ 추가)([../05_data_stores/05_redis_keyspace.md](../05_data_stores/05_redis_keyspace.md) §미확인) | 원본 implementation_plan.md §7.5 · [../05_data_stores/05_redis_keyspace.md](../05_data_stores/05_redis_keyspace.md) |
...
로드맵이 판정에 쓰는 성능 수치와 원본 예상치다. 확정 행을 뺀 나머지는 **미확인 — 확정 전 임의 값 고정 금지**이며 확정은 실험의 실측 결과로만 한다.

| 항목 | 원본 예상치 또는 목표 | 확정 단계 | 확정 자리 |
|------|------|------|------|
| 생성기 단독 처리량 | 20 스레드 머신에서는 미달 가능성이 낮다(원본 서술) | S1 — **닫힘**(워커 1 약 590만 pps · 기록 006 · 410a146 · 부하 실험 · M · 스위치 기본값) | [../10_observability/06_experiment_catalog.md](../10_observability/06_experiment_catalog.md) |
...
| 배치 트리거 세 안의 파트 생성률 | 컨슈머 3개 독립 플러시 시 파트 생성률 3배(원본 산술) | S3 — **닫힘**(A · C 60/분 · B 12~12.5/분 — 창 정렬 fan-in이라 컨슈머 수와 무관 · 기록 020 · fdc7849 · 부하 실험 · M · 스위치 기본값) | 상동 · [../06_pipeline/03_ingest_batch.md](../06_pipeline/03_ingest_batch.md) |
