...
- **분석 무효 구간을 싣지 못한다.** 판정 전수 삽입이 재시도를 소진하면 그 ts 범위가 분석 무효 구간이 되지만([../06_pipeline/08_alarm.md](../06_pipeline/08_alarm.md)) 기록 자리가 정해지지 않아 응답에 표지를 둘 수 없다 — 빈 버킷이 "판정 없음"인지 "기록 실패"인지 가를 수 없다(§미확인 · 미설계 등재).
