...
> **개정일**: 2026-09-24 — W7 보안 판정 반영 — REQ-GLB-24 신설로 불변식 표 밖 REQ-GLB 7 → **8** · 채번 23 → **24**(정본 03_requirements/01 · 12_security/03)
...
db_study는 **로컬 머신 1대**에서 도는 학습 시스템이다. Next.js 웹이 호스트 프로세스로 돌고, NestJS 단일 애플리케이션(api 컨테이너)이 제어 평면과 데이터 평면 모듈을 한 프로세스에 담으며, PostgreSQL 18 · ClickHouse 26.8 · Redis 8이 각자 컨테이너로 선다(D-02 · ADR-01). 스택의 정확 버전은 [../09_tech_stack](../09_tech_stack/README.md)이 갖고 이 문서는 구조만 담는다.
...
- 대응에 쓰인 REQ-GLB(중복 제거): 03 · 04 · 05 · 06 · 08 · 09 · 10 · 11 · 12 · 13 · 14 · 16 · 19 · 20 · 21 · 22 = **16**. 불변식 표에 걸리지 않은 REQ-GLB는 01 · 02 · 07 · 15 · 17 · 18 · 23 · 24 = **8** — 시각 의미론 · 순서 무관성 · 부동소수 비교 · 측정 기록 · 생성 데이터 구분 · 실험 초기 상태 · 쿼리 파라미터 바인딩으로, 구조 요소가 아니라 스키마 · 절차 · 설계 전제가 강제한다. 16 + 8 = **24**(REQ-GLB 채번 합과 일치)
