...
- **파급**: 삽입 횟수가 컨슈머 수 · 티어와 무관하게 시간 트리거로 정해진다 — M · M+ 모두 초당 1회 · 배치 10,000 · 100,000행([07_capacity_planning.md](./07_capacity_planning.md)). Stream 대기가 체류 · 디코딩 · fan-in 대기 셋으로 쪼개진다([05_latency_budget.md](./05_latency_budget.md)). 주의 단계 컨슈머 증설은 삽입 병목에 효과가 없다([06_backpressure_failure.md](./06_backpressure_failure.md)). 기전 정본 [../06_pipeline/03_ingest_batch.md](../06_pipeline/03_ingest_batch.md).
...
- **파급**: PostgreSQL이 멈춰도 시계열 조회는 Dictionary의 마지막 값으로 계속된다. 비활성 태그를 소스에서 거르면 과거 행의 태그명이 사라지는 문제는 [../05_data_stores/07_cross_store_consistency.md](../05_data_stores/07_cross_store_consistency.md)(W3) 판정 대상이다. 계약 REQ-GLB-14.
...
- **파급**: 프로파일을 켠 탐색 수치는 상대 비교용이다. 프로파일 구성원(보정 #17)은 [../09_tech_stack/03_data_infra.md](../09_tech_stack/03_data_infra.md)(W6) 판정 대상 · 메트릭 전수 [../10_observability/01_metrics_catalog.md](../10_observability/01_metrics_catalog.md).
...
- **파급**: 복구 중 새 값과 옛 스풀 값이 섞여 적재되는 것은 순서 무관성이 허용하지만 rt 계열 최신값 덮어쓰기 순서 역전은 [../06_pipeline/05_realtime_read.md](../06_pipeline/05_realtime_read.md)(W4) 판정 대상이다. 상태 머신 3의 하강 전이 잠정 표기를 이 결정으로 확정한다 — [../11_glossary/03_enums_state_machines.md](../11_glossary/03_enums_state_machines.md) 리드 반영. 정본 [06_backpressure_failure.md](./06_backpressure_failure.md).
