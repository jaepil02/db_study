...
**경계 — ingested_at과 alarm_eval.ts의 시간대 표기 통일은 W3 몫이다.** 원본 스키마는 tag_raw.ts에만 시간대 인자를 붙이고 두 컬럼에는 붙이지 않았다. 이 문서는 "시간대 인자는 저장값을 바꾸지 않는다"는 의미를 고정하고, DDL 표기를 어느 쪽으로 맞출지는 [../05_data_stores/03_clickhouse_schema.md](../05_data_stores/03_clickhouse_schema.md)가 결정한다.
...
| alarm:state:{rule_id} | 최초 위반 시각 | 원본 미기재 — epoch ms로 판정 | ms | 없음 |
...
- **alarm:state의 최초 위반 시각을 epoch ms로 둔 것은 판정이다.** 원본은 필드만 적었다. 날짜 문자열로 두면 디바운스 경과 계산(판정 ts − 최초 위반 시각)이 매번 파싱을 거치고, 시간대 없는 문자열이 들어가는 순간 9시간 어긋난 디바운스가 된다. 확정 자리는 [../05_data_stores/05_redis_keyspace.md](../05_data_stores/05_redis_keyspace.md)다.
...
시간대 인자가 실제로 바꾸는 것을 컬럼별로 적는다.

| 동작 | ts(Asia/Seoul) | ingested_at · alarm_eval.ts(인자 없음) | 실패 형태 |
|------|---------------|--------------------------------------|----------|
...
- **미확인 — ClickHouse 서버 시간대 설정.** 원본은 서버 timezone 설정을 적지 않았다. 인자 없는 컬럼 · bucket 컬럼의 달력 경계가 여기에 달려 있다. 확정 자리는 [../09_tech_stack/03_data_infra.md](../09_tech_stack/03_data_infra.md)(W6)이며, 확정 전에는 인자 없는 컬럼의 일 · 월 경계를 신뢰하지 않는다.
...
| tag_1h 버킷 | toStartOfHour(bucket) | bucket 컬럼(인자 없음 — 서버 시간대) | 아니다 — Asia/Seoul은 +09:00 정수 시간이다 |
| tag_1d 버킷 | mv_tag_1d 정의가 원본에 없다 | 미확인 | **움직인다** — 일 경계는 시간대마다 다르다. UTC 자정은 KST 09:00이다 |
| tag_raw 파티션 · TTL | toYYYYMMDD(ts) · toDateTime(ts) + 7일 | Asia/Seoul | 움직인다 — 파티션이 KST 날짜로 잘린다 |
| tag_1m 파티션 | toYYYYMM(bucket) | 서버 시간대 | 움직인다 — 서버가 UTC면 월 파티션이 KST 1일 09:00에 넘어간다 |
| alarm_eval 파티션 | toYYYYMMDD(ts) | 서버 시간대 | 움직인다 |
| alarm_event 월 파티션 | occurred_at 월 RANGE(pg_partman) | 파티션 경계값의 해석 시간대 | 움직인다 — 확정 자리는 [../05_data_stores/02_postgresql_constraints.md](../05_data_stores/02_postgresql_constraints.md) |

- **분 · 시 롤업은 안전하고 일 롤업 · 파티션은 위험하다.** tag_1d의 "하루"가 KST 자정 기준인지 UTC 자정 기준인지 정하지 않으면, 일별 생산 추이 화면의 하루가 오전 9시에 시작한다. mv_tag_1d의 시간대 명시는 [../05_data_stores/04_clickhouse_rollup.md](../05_data_stores/04_clickhouse_rollup.md)(W3)가 확정한다.
- **tag_raw는 KST 날짜로, tag_1m은 서버 시간대 월로 파티션된다.** 같은 순간의 원시 행과 롤업 행이 서로 다른 달력으로 잘리므로, 보존 기간 경계에서 원시가 먼저 사라지는 시각을 "자정"으로 말할 때 어느 자정인지 밝힌다. 보존 정본은 [../05_data_stores/08_retention_lifecycle.md](../05_data_stores/08_retention_lifecycle.md)다.
...
- **미확인 — site.timezone의 용도.** 원본 ERD의 site 테이블에 timezone 컬럼이 있다(원본 architecture.md §6). 표시를 Asia/Seoul 단일로 고정한 현행 규칙과 사이트별 시간대 컬럼의 관계는 원본에 없다. 확정 자리는 [../05_data_stores/01_postgresql_schema.md](../05_data_stores/01_postgresql_schema.md)(W3)이며, 확정 전에는 이 컬럼이 표시에 쓰이지 않는다.
...
- [../05_data_stores/03_clickhouse_schema.md](../05_data_stores/03_clickhouse_schema.md) — 시각 컬럼 DDL · 시간대 표기 통일(W3)
