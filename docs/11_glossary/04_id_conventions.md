...
- **API 표면 번호의 표기는 "{문서} #N"(공백 1칸)이다.** [../README.md](../README.md) ID 규약 표의 형식 열은 {문서}-#N으로, 예 열은 05_timeseries #3으로 서로 다르게 적혀 있다. 이 문서는 예 열을 따른다 — 하이픈을 넣으면 화면 코드({표면}-{의미})와 같은 모양이 되어 grep 한 번으로 둘을 가를 수 없다.
