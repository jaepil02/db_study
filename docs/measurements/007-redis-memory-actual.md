# 007 — Stream 엔트리 인코딩 크기: 태그/설비 50(S 구성) · 신호 프로파일별 바이트 (EXP-39의 S1 몫)

> 실험: EXP-39 · 상태: 유효 · 정정 대상: 없음 · 판정 창: 2026-09-24T13:21:56Z ~ 2026-09-24T13:24:01Z(티어 S · M · L 한 묶음으로 실행 · 이 기록은 S 조건 6회)
...
| 용량 티어 | S — 설비당 태그 50(04_architecture/07) · 설정 로더 CAPACITY_TIER가 생성 규모와 기록 칸의 한 원천 |
...
| 관측 스택 · 생성기 CPU · CPU 배치 | off · 최대 1.80코어 · datagen 위치 cpuset 11-12 · 워커 2 |
| 창 | 워밍업 1초 · 창 5초 — 크기 측정이라 짧은 창으로 충분하다(3회 편차 0.15%) |
| 압축 · swap · 네트워킹 · SIM 계획 | 해당 없음 · 사용 없음 · 해당 없음(macOS Docker Desktop) · 없음 |
| 스크립트 | scripts/lab/s1/gen-bench.sh <출력> datagen S <mixed · RANDOM_WALK> 2 3 5 1 |

## 결과

| 태그/설비 | 신호 프로파일 | 엔트리당 바이트 3회 | 중앙값 | 포인트당 바이트 |
|------|------|------|------|------|
| 50(S) | 혼합 | 518 · 518 · 518 | **518** | 10.36 |
| 50(S) | RANDOM_WALK | 680 · 679 · 680 | **680** | 13.59 |
...
```json
{
  ...
  "conditions": {
    ...
    "script": "scripts/lab/s1/gen-bench.sh <출력> datagen S <mixed|RANDOM_WALK> 2 3 5 1"
  },
  "repeat": {
    "runs": 3,
    "deviation": 0.0015,
