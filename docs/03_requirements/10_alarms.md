...
| **REQ-ALM-16** | 확인된 알람의 조건이 해소되면 행을 CLEARED로 닫고 acked_by · acked_at을 유지한다. ACKED → NORMAL은 해제 디바운스 없이 첫 해소에서 전이한다(원본 사실). 확인은 alarm:state를 쓰지 않고 판정기가 해소 첫 감지 때 acked_at을 읽어 이 결과를 지킨다(W4 판정) | 원본 data_flow.md §8.1 · [../11_glossary/03_enums_state_machines.md](../11_glossary/03_enums_state_machines.md) · ALM-04 · ALM-08 | Redis 상태가 ACTIVE로 남으면 확인된 알람도 CLEARING 디바운스를 타 상태도의 ACKED → NORMAL이 실행되지 않고 해제 시각이 디바운스만큼 늦게 기록된다 | 확인 후 해소 1회 → 행 CLEARED · acked 값 유지 · cleared_at − 해소 ts 대조 | ALM-04 · ALM-08 | F-06 | 해당 없음 |
...
- CLEARING 중 확인은 alarm:state를 바꾸지 않는다(W4 판정 — CLEARING 유지 · 디바운스 완료에서 닫기 · 그 사이 재위반하면 ACTIVE로 돌아간 뒤 다음 해소에서 즉시 닫기 · [../06_pipeline/08_alarm.md](../06_pipeline/08_alarm.md) §ACK와 alarm:state). 이 문서가 고정하는 것은 행의 결과(REQ-ALM-16)다.
...
| 확인이 alarm:state를 바꾸는 주체 · CLEARING 중 확인의 Redis 전이 | 확인은 API가 PostgreSQL에 쓴다 | **W4 판정** — 판정기 단독 · CLEARING 중 확인은 Redis 전이 없음 | [../06_pipeline/08_alarm.md](../06_pipeline/08_alarm.md) |
...
| 비활성 태그의 열린 이벤트를 닫는 수단 | 없다 | 닫힘 — 닫는 수단을 두지 않는다(W5 판정 — 알람 강제 해제 표면 없음 · [../07_api/README.md](../07_api/README.md) 원본에 없는 표면 행). 시스템은 거짓 해제를 막으려 닫지 않고 사람의 확인(ALM-08)만 남는다 · 잔여는 한계 등재 #17 | [../06_pipeline/08_alarm.md](../06_pipeline/08_alarm.md) · [../02_features/09_alarms.md](../02_features/09_alarms.md) |
