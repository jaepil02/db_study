...
| 항목 | 상태 | 확정 자리 |
|------|------|------|
| 레이트 리밋의 엔드포인트 차원 | **불일치** — "엔드포인트별로 엄격히"(원본 architecture.md §18)와 키 rl:{user_id}:{unix_minute}(원본 architecture.md §8.2)에 엔드포인트 자리가 없다 | [../05_data_stores/05_redis_keyspace.md](../05_data_stores/05_redis_keyspace.md)(W3) · [../12_security/03_api_surface_defense.md](../12_security/03_api_surface_defense.md) |
| 권한 캐시 키 모양 | 키 이름 없음 | [../05_data_stores/05_redis_keyspace.md](../05_data_stores/05_redis_keyspace.md)(W3) |
| sess:{session_id} 소비 기능 | 소비 기능 없음 — 이 문서의 어떤 REQ도 세션 키를 쓰지 않는다 | 상동 |
