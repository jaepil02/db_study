...
| **REQ-MST-03** | modbus_config는 설비와 1:1이며 host · port · unit_id · timeout_ms · retry_count · max_regs_per_request를 갖는다. **host가 컨테이너 루프백이면 그 설비는 시뮬레이션 설비**이고 Collector는 그 설비의 정상 값에 SIMULATED(9)를 단다. 접속 설정 변경은 Collector 재기동 전까지 폴링에 반영되지 않는다(현행) | 원본 architecture.md §6 · 원본 data_flow.md §3 · REQ-GLB-18 · [../02_features/03_collector.md](../02_features/03_collector.md) §모드 A의 SIMULATED 표지 판정 | host를 루프백에서 실장비 주소로 바꾸고 이력을 남기지 않으면 과거 행의 출처 복원(device_id → host)이 틀린다. 재기동 없이 반영된다고 믿으면 변경 직후 측정이 옛 설정으로 돈다 | host 루프백 설비의 적재 quality 분포 조회(정상 값 9) · 설정 변경 후 재기동 전후 폴링 대상 비교 | MST-03 | F-01 · F-05 | common.validation_failed/400 |
...
| 실행 중 마스터 변경의 Collector 반영 | 기동 시 1회 로드뿐 — REQ-MST-03이 현행을 적었다 | [../06_pipeline/02_collect.md](../06_pipeline/02_collect.md)(W4) |
