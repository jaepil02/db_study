...
이 문서는 **외부 URL의 유일한 등재처**다. 다른 문서는 URL을 쓰지 않고 이 문서를 링크하며, 무엇을 확인해야 하는지만 적는다([../CLAUDE.md](../CLAUDE.md) 외부 URL 규칙). URL을 한 자리에 모으는 이유는 하나다 — 공식 문서의 주소는 버전마다 바뀌고, 여러 문서에 흩어진 URL은 한 번에 고칠 수 없어 낡은 주소가 조용히 남는다.
