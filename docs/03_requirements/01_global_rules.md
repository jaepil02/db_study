...
- **ClickHouse 서버 시간대 설정은 미확인이다.** 확정 전에는 시간대 인자 없는 컬럼(ingested_at · bucket · alarm_eval.ts)의 일 · 월 경계를 판정 근거로 쓰지 않는다 — 확정 자리 [../09_tech_stack/03_data_infra.md](../09_tech_stack/03_data_infra.md)(W6).
...
| 17 | 로컬 전용 | REQ-GLB-19 | 1 |

- **불변식 17행 전부 대응이 있다 — 누락 0.** 대응이 둘인 불변식 3행(시각 의미론 · 비동기 경계 · 측정 기록)은 요약 문장 하나가 서로 다른 실패 두 개를 막고 있어 행을 갈랐다.
- 불변식에 걸리지 않은 REQ-GLB 3행(20 · 21 · 22)은 원본 설계 원칙에서 왔다 — §원본 설계 원칙 대응이 닫는다.

### 검산

- 세는 기준은 **불변식 행별 대응 REQ 수의 합**이다. 대응 둘 3행(1 · 2 · 15) + 대응 하나 14행 = 3 × 2 + 14 × 1 = **20**
- 불변식에 걸린 REQ-GLB = 01~19 · 23 = 19 + 1 = **20**. REQ 하나가 불변식 둘에 걸린 경우가 없어 두 수가 같다
- REQ-GLB 채번 합 = 불변식 대응 20 + 원칙 전용 3(20 · 21 · 22) = **23**
